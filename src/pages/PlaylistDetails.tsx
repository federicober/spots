...
export default function PlaylistDetails() {
    ...
    const { playlistId } = useParams<{ playlistId: string }>() as Record<string, string>;
    ...
    function stageTrack(track: Track) {
        setStagedTracks((
            prevStagedTracks) => [...prevStagedTracks, track]
        )
    }
    function unstageTrack(track: Track) {
        setStagedTracks((
            prevStagedTracks) => prevStagedTracks.filter(
        ...
        async function fetchData() {
            if (playlist !== null) {
                if (playlist.tracks.total <= playlist.tracks.limit) {
                    setTracks(playlist.tracks.items.map(addedtrack => addedtrack.track))
                }
                else {
                    // TODO
                    setTracks(playlist.tracks.items.map(addedtrack => addedtrack.track))
        ...
        <Stack spacing={10} direction={{ sm: 'column', md: 'row' }}>
            ...
            <Item>
                ...
                <List>
                    ...
                    {tracks.sort(
                        (a, b) => b.artists[0].name.localeCompare(a.artists[0].name)
                    ).filter(
                        track => track.name.toLowerCase().includes(searchedTrack)
                    ).map(track => (
                        <TrackListItem
                            key={track.id}
                            track={track}
